...
class ResNetSE(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        #print("3", x.shape)# B, 16, 20, 302 => B X 16 X 40 X 202
        x = self.bn1(x)
        #print("4", x.shape)# 400, 16, 20, 202 => B X 16 X 20 X 202
        x = self.relu(x)
        #print("5", x.shape)# 400, 16, 20, 202 => B X 16 X 20 X 202
        x = self.layer1(x)
        #print("6", x.shape)# 400, 16, 20, 202 => B X 16 X 20 X 202
        x = self.layer2(x)
        #print("7", x.shape)#400, 32, 10, 101 => B X 32 X 10 X 202
        x = self.layer3(x)
        #print("8", x.shape)# 400, 64, 5, 51 => B X 64 X 5 X 51
        x = self.layer4(x)
        #print("9", x.shape)# 400, 128, 5, 51 => B X 64 X 5 X 51 변화점
        x = torch.mean(x, dim=2, keepdim=True)
        #print("10", x.shape)# 400, 64, 1, 51 => B X 64 X 1 X 51
        ...
        if True in torch.isnan(global_x):
            print("NAN!!")
        #print("Global_", global_x.shape) #400, 384, 51 => B, 192, 51
